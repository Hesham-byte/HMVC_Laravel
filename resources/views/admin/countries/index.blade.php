...
@section('content')
    ...
    <nav class="breadcrumb">
        ...
        <span class="breadcrumb-item active">{{__('main.countries')}}</span>
    </nav>

    <div class="card">
        <div class="card-body">
            <a href="{{route('admin.countries.create')}}" class="btn btn-outline-primary mb-1 float-right"><i class="fa fa-plus"></i> {{__('main.create')}}</a>
...
<script>
    $(function() {
        $('#countries-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{route('admin.countries.get-data',request()->all())}}",
            columns:[
                {data:'id',name:'id'},
                {data:'name',name:'name'},
