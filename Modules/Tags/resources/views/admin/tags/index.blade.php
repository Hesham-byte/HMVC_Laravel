...
@section('content')
    ...
    <nav class="breadcrumb">
        ...
        <span class="breadcrumb-item active">{{__('tags::common.tags')}}</span>
    </nav>

    <div class="card">
        <div class="card-body">
            <a href="{{route('admin.tags.create')}}" class="btn btn-outline-primary mb-1 float-right"><i class="fa fa-plus"></i> {{__('main.create')}}</a>
...
<script>
    $(function() {
        $('#tags-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{route('admin.tags.get-data',request()->all())}}",
            columns:[
                {data:'id',name:'id'},
                {data:'name',name:'name'},
