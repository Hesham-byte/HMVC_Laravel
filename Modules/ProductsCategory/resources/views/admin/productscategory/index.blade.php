...
@section('content')
    ...
    <nav class="breadcrumb">
        ...
        <span class="breadcrumb-item active">{{__('productscategory::common.productscategory')}}</span>
    </nav>

    <div class="card">
        <div class="card-body">
            <a href="{{route('admin.productscategory.create')}}" class="btn btn-outline-primary mb-1 float-right"><i class="fa fa-plus"></i> {{__('main.create')}}</a>

            <table class="table" id="productscategory-table">
                <thead>
                    <tr>
                        <th>{{__('main.id')}}</th>
                        <th>{{__('users.name')}}</th>
...
<script>
    $(function() {
        $('#productscategory-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{route('admin.productscategory.get-data',request()->all())}}",
            columns:[
                {data:'id',name:'id'},
                {data:'name',name:'name'},
