...
@section('content')
    ...
    <nav class="breadcrumb">
        ...
        <span class="breadcrumb-item active">{{__('banners::common.banners')}}</span>
    </nav>

    <div class="card">
        <div class="card-body">
            <a href="{{route('admin.banners.create')}}" class="btn btn-outline-primary mb-1 float-right"><i class="fa fa-plus"></i> {{__('main.create')}}</a>
...
<script>
    $(function() {
        $('#banners-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{route('admin.banners.get-data',request()->all())}}",
            columns:[
                {data:'id',name:'id'},
                {data:'name',name:'name'},
