...
<div class="row">
    ...
    <div class="col-md-6 form-group">
        <label for="">{{__('main.status')}}</label>
        <select name="status" class="form-control select-2">
            <option value="">{{__('main.select')}}</option>
            @foreach(['active', 'pending'] as $status)
                <option value="{{$status}}" @selected($model->status==$status)>{{__('main.'.$status)}}</option>
            @endforeach
        </select>
    </div>

    <div class="col-md-6 form-group">
        <label for="">{{__('main.show_in_app')}}</label>
        <select name="show_in_app" class="form-control select-2">
            <option value="">{{__('main.select')}}</option>
            @foreach([1, 0] as $show_in_app)
                <option value="{{$show_in_app}}" @selected($model->show_in_app==$show_in_app)>{{__('main.'.$show_in_app)}}</option>
            @endforeach
        </select>
    </div>
</div>
